...
public class GrpcHelloWorldTest extends BaseGrpcTest {
    ...
    public void testGrpcHelloWorld() throws Exception {
        Descriptors.FileDescriptor helloWorldProto = parseProtoFileWithDependencies(TestResources.ProtoFiles.HelloWorld.file(), Set.of());
    ...
    private static void validateResponse(GrpcServerRequest<Buffer, Buffer> request, Buffer protoHelloBuffer, BlockingQueue<Buffer> requestsReceived, Buffer replyBuffer) {
    ...
    public static Message decodeWith(Descriptors.Descriptor messageDescriptor, Buffer encodedData) throws Exception {
        ...
        builder.mergeFrom(nettyBuffer.array(), nettyBuffer.arrayOffset() + nettyBuffer.readerIndex(), nettyBuffer.readableBytes());
