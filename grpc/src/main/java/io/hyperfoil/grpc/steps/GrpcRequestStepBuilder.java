...
public class GrpcRequestStepBuilder extends BaseStepBuilder<GrpcRequestStepBuilder> {
    ...
    public List<Step> build() {
        ...
        var sendRequest = new GrpcSendRequestStep(stepId, call, data, requestCtxKey, metadataAppenders.toArray(new SerializableBiConsumer[0]), cacheable);
        var receivedResponseHeader = new GrpcReceivedResponseHeaderStep(requestCtxKey);
        var receivedResponseData = new GrpcReceivedResponseDataStep(requestCtxKey);
        return List.of(prepareRequest, sendRequest, receivedResponseHeader, receivedResponseData);
    }
}
